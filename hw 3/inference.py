...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        In order to obtain the distribution over new positions forthe
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions: #p is old position
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #             allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]
        allPossible = util.Counter()
        if noisyDistance == None:#all go to jail!
            newBeliefs = []
            ...
            if allPossible.totalCount() == 0:
                ...
                newBelief = []
                for i in range(0, self.numParticles):
                    newBelief.append(util.sample(allPossible))
    ...
    def elapseTime(self, gameState):
        ...
        for p in range(0, self.numParticles):
            tempPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.beliefs[p]))
            newPosDist.append(util.sample(tempPosDist))
