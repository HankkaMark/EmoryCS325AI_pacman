...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # successorPosition = Actions.getSuccessor(position, action)
        ghost_minDist = 999999
        hunt_target = None
        nextaction = Directions.STOP
        act_minDist = 999999
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            #for each ghost that lives, find position;find nearest one and hunt
            for i in range(1, len(livingGhosts)):
                if livingGhosts[i] == True:
                    maxPos = None
                    maxp = 0
                    for beliefs in livingGhostPositionDistributions:
                        for posX, posY in beliefs:
                            pos = (posX, posY)
                            if beliefs[pos] > maxp:
                                maxPos = pos
                                maxp = beliefs[pos]
                    mazedistance = self.distancer.getDistance(maxPos, pacmanPosition)
                    if mazedistance < ghost_minDist:
                        ghost_minDist = mazedistance
                        hunt_target = maxPos
            distance = self.distancer.getDistance(successorPosition, hunt_target)
            if distance < act_minDist:
                nextaction = action
                act_minDist = distance
        return nextaction
